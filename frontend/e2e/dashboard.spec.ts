...
test.describe("Dashboard", () => {
  ...
  test("should switch between tabs", async ({ page }) => {
    ...
    await expect(page.getByRole("tab", { name: "Strategy" })).toHaveAttribute(
      "data-state",
      "active",
    );
    ...
    await expect(page.getByRole("tab", { name: "Backtesting / ML" })).toHaveAttribute(
      "data-state",
      "active",
    );
  ...
  test("should require confirmation for flatten and kill actions", async ({ page }) => {
    ...
    expect(
      dialogs.some((d) => d.type === "confirm" && d.message.includes("Flatten will attempt")),
    ).toBeTruthy();
    expect(
      dialogs.some(
        (d) => d.type === "prompt" && d.message.includes("Emergency stop requires a reason"),
      ),
    ).toBeTruthy();
    expect(
      dialogs.filter((d) => d.type === "confirm" && d.message.includes("EMERGENCY STOP")).length,
