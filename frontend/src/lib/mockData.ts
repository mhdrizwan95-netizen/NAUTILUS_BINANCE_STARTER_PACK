...
import type { Venue, Strategy, StrategyPerformance, Trade, Alert, GlobalMetrics, VenueType } from '../types/trading';
  ...
  strategies.forEach(strategy => {
    venues.forEach(venue => {
      ...
      const isCompatible = 
        ...
        (venue.type === 'equities' && ['MeanRev', 'Breakout', 'Momentum'].includes(strategy.name)) ||
      ...
      performances.push({
        ...
        metrics: {
          ...
          topSymbols: [
            { symbol: venue.type === 'crypto' ? 'BTC/USDT' : venue.type === 'equities' ? 'AAPL' : 'EUR/USD', pnl: Math.random() * 1000 },
            { symbol: venue.type === 'crypto' ? 'ETH/USDT' : venue.type === 'equities' ? 'TSLA' : 'GBP/USD', pnl: Math.random() * 800 },
            { symbol: venue.type === 'crypto' ? 'SOL/USDT' : venue.type === 'equities' ? 'NVDA' : 'USD/JPY', pnl: Math.random() * 600 },
          ],
