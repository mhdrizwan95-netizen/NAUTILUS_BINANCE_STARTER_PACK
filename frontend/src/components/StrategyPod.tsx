...
export function StrategyPod({ performance, strategy, venue, onClick }: StrategyPodProps) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`relative rounded-2xl bg-zinc-900/70 backdrop-blur-xl border ${healthColor} p-4 cursor-pointer group overflow-hidden`}
    >
      {/* Gradient glow on hover */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${venueGradient} opacity-0 group-hover:opacity-5 transition-opacity duration-300 pointer-events-none`}
      />
      ...
      <div className="flex items-start justify-between mb-3 relative z-10">
        ...
        <div className="flex flex-col items-end">
          <div className={`flex items-center gap-1 ${getPnLColor()}`}>
            {getTrendIcon()}
            <span className="font-mono">{formatPercent(metrics.pnlPercent)}</span>
          </div>
          <span className={`text-xs font-mono ${getPnLColor()}`}>
