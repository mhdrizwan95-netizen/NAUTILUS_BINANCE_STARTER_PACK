...
#### Control & Auth
Set `OPS_API_TOKEN` directly (or provide a file via `OPS_API_TOKEN_FILE`, e.g. mounted from `${SECRETS_DIR}/ops_api_token`) before bringing the stack up. The following Ops endpoints require `X-Ops-Token` with the value of `OPS_API_TOKEN` (and will log the optional `X-Ops-Actor` call-sign for audit trails):
