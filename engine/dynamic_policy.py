...
def per_trade_risk_pct(mode: Mode, strat: StrategyContext) -> float:
    base = {
        ("scalp", "red"):    0.004, ("scalp", "yellow"): 0.008, ("scalp", "green"):  0.012,
        ("momentum","red"):  0.006, ("momentum","yellow"):0.012, ("momentum","green"): 0.018,
        ("trend","red"):     0.007, ("trend","yellow"):  0.015, ("trend","green"):   0.022,
        ("event","red"):     0.003, ("event","yellow"):  0.007, ("event","green"):   0.012,
    }[(strat.type, mode)]
    tf_adj = {"1m":-0.0015,"5m":-0.001,"15m":0.0,"1h":0.001,"4h":0.002}[strat.base_timeframe]
...
def dynamic_position_notional_usd(mode: Mode, strat: StrategyContext, mkt: MarketSnapshot, acct: AccountState) -> Tuple[float, float]:
    stop_pct = target_stop_pct(strat, mkt)
    risk_pct = per_trade_risk_pct(mode, strat)
    free_risk = max(0.0, (0.10 if mode=="green" else 0.06 if mode=="yellow" else 0.03) - acct.open_risk_sum_pct)
