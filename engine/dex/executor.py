...
class DexExecutor:
    ...
    def _path_buy(self, token_address: str) -> Sequence[str]:
        ...
        if self.wrapped_native and self.wrapped_native.lower() not in {stable_cs.lower(), token_cs.lower()}:
            return (stable_cs, self.wallet.w3.to_checksum_address(self.wrapped_native), token_cs)
    ...
    def _path_sell(self, token_address: str) -> Sequence[str]:
        ...
        stable_cs = self.wallet.w3.to_checksum_address(self.stable_token)
        if self.wrapped_native and self.wrapped_native.lower() not in {stable_cs.lower(), token_cs.lower()}:
            return (token_cs, self.wallet.w3.to_checksum_address(self.wrapped_native), stable_cs)
