...
def score_symbols(features: List[SymbolFeatures]) -> Dict[str, float]:
    ...
    mvol = sum(vols)/len(vols); s_vol = (sum((v-mvol)**2 for v in vols)/max(1,len(vols)-1))**0.5
    mdep = sum(depths)/len(depths); s_dep = (sum((d-mdep)**2 for d in depths)/max(1,len(depths)-1))**0.5
    mspr = sum(spreads)/len(spreads); s_spr = (sum((s-mspr)**2 for s in spreads)/max(1,len(spreads)-1))**0.5
    matr = sum(atrs)/len(atrs); s_atr = (sum((a-matr)**2 for a in atrs)/max(1,len(atrs)-1))**0.5
