...
venue_trades = Counter(
    "venue_trades_total", "Trades fetched from venue (applied via reconciliation)"
)
...
market_value_usd = Gauge(
    "market_value_usd",
    "Signed market value of open positions in USD",
    multiprocess_mode="max",
)
initial_margin_usd = Gauge(
    "initial_margin_usd",
    "Initial margin currently in use (USD)",
    multiprocess_mode="max",
)
maint_margin_usd = Gauge(
    "maint_margin_usd", "Maintenance margin requirement (USD)", multiprocess_mode="max"
)
available_usd = Gauge(
    "available_usd",
    "Available balance to trade/withdraw (USD)",
    multiprocess_mode="max",
)
snapshot_id = Gauge(
    "snapshot_id",
    "Sequential snapshot counter for exporter loop",
    multiprocess_mode="max",
)
max_notional_usdt = Gauge(
    "max_notional_usdt", "Current MAX_NOTIONAL_USDT limit", multiprocess_mode="max"
)
trading_enabled_gauge = Gauge(
    "trading_enabled", "1 if trading enabled, else 0", multiprocess_mode="max"
)
snapshot_loaded_gauge = Gauge(
    "snapshot_loaded",
    "1 if a portfolio snapshot is loaded, else 0",
    multiprocess_mode="max",
)
mark_time_epoch = Gauge(
    "mark_time_epoch",
...
pnl_unrealized_symbol = Gauge(
    "pnl_unrealized_symbol",
    "Unrealized PnL by symbol (USD)",
    ["symbol"],
    multiprocess_mode="max",
)

# Per-symbol position/basis metrics for dashboard tables
position_amt_by_symbol = Gauge(
    "position_amt",
    "Position amount (signed base units)",
    ["symbol"],
    multiprocess_mode="max",
)
entry_price_by_symbol = Gauge(
    "entry_price", "Average entry price (USD)", ["symbol"], multiprocess_mode="max"
)
...
venue_exposure_usd = Gauge(
    "venue_exposure_usd",
    "Aggregate exposure per venue (USD)",
    ["venue"],
    multiprocess_mode="max",
)
risk_equity_buffer_usd = Gauge(
    "risk_equity_buffer_usd",
    "Equity buffer above configured floor (USD)",
    multiprocess_mode="max",
)
risk_equity_drawdown_pct = Gauge(
    "risk_equity_drawdown_pct",
    "Peak-to-trough equity drawdown percent",
    multiprocess_mode="max",
)
risk_equity_floor_breach = Gauge(
    "risk_equity_floor_breach",
    "1 if equity breaker currently tripped",
    multiprocess_mode="max",
)
exposure_cap_usd = Gauge(
    "exposure_cap_usd",
    "Configured exposure cap per venue (USD)",
    ["venue"],
    multiprocess_mode="max",
)
risk_equity_floor_usd = Gauge(
    "risk_equity_floor_usd", "Configured equity floor (USD)", multiprocess_mode="max"
)
risk_equity_drawdown_limit_pct = Gauge(
    "risk_equity_drawdown_limit_pct",
    "Configured equity drawdown breaker percent",
    multiprocess_mode="max",
)
risk_metrics_freshness_sec = Gauge(
    "risk_metrics_freshness_sec",
    "Seconds since last risk telemetry refresh",
    multiprocess_mode="max",
)
risk_exposure_headroom_usd = Gauge(
...
metrics_heartbeat = Gauge(
    "metrics_heartbeat",
    "Timestamp of last metrics system update",
    multiprocess_mode="max",
)
...
def update_portfolio_gauges(
    cash: float, realized: float, unrealized: float, exposure: float
) -> None:
