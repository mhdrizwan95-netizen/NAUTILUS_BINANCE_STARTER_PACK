        ...
        try:
            ...
            if not prot.has_stop_for(side):
                ...
                if bool(self.cfg.get("STOP_VALIDATOR_REPAIR", False)):
                    await self.router.amend_stop_reduce_only(symbol, "SELL" if side == "LONG" else "BUY", want_sl, abs(qty))
                    try:
                        self.metrics.stop_validator_repaired_total.labels(symbol=symbol, kind="SL").inc()
                    except Exception:
                        pass
                    ...
                    if bool(self.cfg.get("STOPVAL_NOTIFY_ENABLED", False)) and self.bus is not None:
                        ...
                        if now - last >= int(self.cfg.get("STOPVAL_NOTIFY_DEBOUNCE_SEC", 60)):
                            try:
                                self.bus.fire("notify.telegram", {"text": f"\ud83d\udee1\ufe0f Stop repaired on *{symbol}* at `{want_sl}`"})
