...
def _flush_loop(con: sqlite3.Connection):
    ...
    while True:
        ...
        if batch and (len(batch) >= 64 or time.time() - last > 0.5):
            with _LOCK:
                con.executemany(batch[0][0], [p for _, p in batch]) if len({sql for sql,_ in batch})==1 \
...
def insert_order(d: Dict[str, Any]):
    enqueue("""INSERT OR REPLACE INTO orders(id,venue,symbol,side,qty,price,status,ts_accept,ts_update)
               VALUES(?,?,?,?,?,?,?,?,?)""",
            (d["id"], d["venue"], d["symbol"], d["side"], d["qty"], d.get("price"),
             d["status"], d["ts_accept"], d["ts_update"]))

def insert_fill(d: Dict[str, Any]):
    enqueue("""INSERT OR REPLACE INTO fills(id,order_id,venue,symbol,side,qty,price,fee_ccy,fee,ts)
               VALUES(?,?,?,?,?,?,?,?,?,?)""",
            (d["id"], d["order_id"], d["venue"], d["symbol"], d["side"], d["qty"],
             d["price"], d.get("fee_ccy"), d.get("fee"), d["ts"]))

def upsert_position(venue: str, symbol: str, net_qty: float, avg_price: float | None, ts_ms: int):
    enqueue("""INSERT INTO positions(venue,symbol,net_qty,avg_price,updated_ms)
               VALUES(?,?,?,?,?)
...
def insert_equity(venue: str, equity: float, cash: float, upnl: float, ts_ms: int):
    enqueue("""INSERT OR REPLACE INTO equity_snapshots(venue,equity_usd,cash_usd,upnl_usd,ts)
               VALUES(?,?,?,?,?)""", (venue, equity, cash, upnl, ts_ms))
