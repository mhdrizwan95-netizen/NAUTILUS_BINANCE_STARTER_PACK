...
import type { StrategyPerformance, Strategy, Venue, Trade } from "@/lib/types";
...
export function RightPanel({ performance, strategy, venue, trades, onClose }: RightPanelProps) {
  ...
  const equityCurveData = performance.metrics.sparkline.map((value, index) => ({ index, value }));
  const confidenceData = Array.from({ length: 24 }, (_, i) => ({ time: i, confidence: 0.5 + Math.random() * 0.4 }));
  ...
  const strategyTrades = trades.filter((t) => t.strategyId === strategy.id && t.venueId === venue.id);
          ...
          <div className="p-6 space-y-6">
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                />
                <MetricCard label="Win Rate" value={`${(performance.metrics.winRate * 100).toFixed(1)}%`} color="text-cyan-400" />
                <MetricCard label="Sharpe Ratio" value={performance.metrics.sharpe.toFixed(2)} color="text-violet-400" />
                <MetricCard label="Max Drawdown" value={`${(performance.metrics.drawdown * 100).toFixed(1)}%`} color="text-amber-400" />
              </div>
            </section>
              ...
              <div className="h-32 bg-zinc-800/30 rounded-xl p-4">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={equityCurveData}>
                    ...
                    <Area type="monotone" dataKey="value" stroke={venueColor} fill="url(#equityGradient)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </section>
              ...
              <div className="h-24 bg-zinc-800/30 rounded-xl p-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={confidenceData}>
                    <Line type="monotone" dataKey="confidence" stroke={venueColor} strokeWidth={2} dot={false} />
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-2">
                {performance.metrics.topSymbols.map((symbol, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-zinc-800/30">
                    ...
                    <span className={clsx("text-xs font-mono", symbol.pnl >= 0 ? "text-emerald-400" : "text-red-400")}>{formatCurrency(symbol.pnl)}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-2">
                {strategyTrades.slice(0, 8).map((trade) => (
                  <div key={trade.id} className="flex items-center justify-between p-3 rounded-lg bg-zinc-800/30">
                    <div>
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-2">
                <HealthItem label="Connection" status={venue.status === "connected" ? "optimal" : "warning"} value={`${venue.latency}ms`} />
                <HealthItem label="Risk Exposure" status={performance.metrics.var < 0.02 ? "optimal" : "warning"} value={`${(performance.metrics.var * 100).toFixed(1)}%`} />
                <HealthItem label="Trade Frequency" status="optimal" value={`${performance.metrics.tradeCount} / day`} />
              </div>
            </section>
          </div>
        </ScrollArea>
...
function MetricCard({
  ...
  return (
    <div className="bg-zinc-800/30 rounded-xl p-3">
      ...
      <div className={clsx("font-mono mb-0.5", color)}>{value}</div>
