...
const generateSparkline = (
  trend: "up" | "down" | "volatile" | "flat" = "volatile",
): number[] => {
  const points = 24;
  const data: number[] = [];
  let value = 100;
...
export const generatePerformanceData = (): StrategyPerformance[] => {
  ...
  strategies.forEach((strategy) => {
    venues.forEach((venue) => {
      const isCompatible =
        (venue.type === "crypto" && ["HMM", "Breakout"].includes(strategy.name)) ||
        (venue.type === "equities" && ["MeanRev", "Breakout"].includes(strategy.name));
      ...
      performances.push({
        ...
        metrics: {
          ...
          topSymbols: [
            ...
            {
              symbol: venue.type === "crypto" ? "SOL/USDT" : "NVDA",
              pnl: Math.random() * 600,
            },
          ],
...
export const generateRecentTrades = (count: number = 20): Trade[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    trades.push({
      ...
      symbol: venue.type === "crypto" ? "BTC/USDT" : "AAPL",
...
export const generateAlerts = (count: number = 10): Alert[] => {
  ...
  const messages = [
    ...
    { type: "warning" as const, msg: "Latency increased on IBKR connection" },
...
export const getGlobalMetrics = (
  ...
  return {
    ...
    sharpe:
      performances.reduce((sum, p) => sum + p.metrics.sharpe, 0) /
