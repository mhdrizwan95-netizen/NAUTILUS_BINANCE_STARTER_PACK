    ...
    if last_train_ms == 0 or now_ms - last_train_ms >= period_ms:
        ...
        try:
            with httpx.Client(timeout=60) as x:
                r = x.post(f"{settings.ML_SERVICE}/train", json={"n_states": 4, "promote": settings.PROMOTE})
...
def _get_params(strategy: str, instrument: str, features: dict) -> dict:
    try:
        with httpx.Client(timeout=10) as x:
            r = x.get(f"{settings.PARAM_CONTROLLER}/param/{strategy}/{instrument}", params={**{f"features[{k}]":v for k,v in features.items()}})
            if r.status_code == 200:
                return r.json()
    except Exception as e:
        logger.warning(f"param get failed: {e}")
    return {"config_id":"na","params":{},"policy_version":"na","features_used":[]}
...
def run():
    ...
    while step < settings.MAX_STEPS:
        ...
        for _, row in bars.iterrows():
            ...
            bar = {"timestamp": ts, "open": float(row["open"]), "high": float(row["high"]), "low": float(row["low"]), "close": float(row["close"]), "volume": float(row.get("volume",0.0))}
            ...
            equity_val = (equity[-1]["equity"] if equity else 100_000.0) + mtm  # mark-to-market P&L on top of base equity
