...
def upsert_preset(
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO presets(strategy,instrument,preset_id,params_json)
                   VALUES(?,?,?,?)
...
def list_presets(
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT preset_id, params_json FROM presets WHERE strategy=? AND instrument=? ORDER BY preset_id ASC",
        (strategy, instrument),
    )
...
def log_outcome(
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO outcomes(ts,strategy,instrument,preset_id,reward,features_json) VALUES(?,?,?,?,?,?)",
        (
            time.time(),
            strategy,
            instrument,
