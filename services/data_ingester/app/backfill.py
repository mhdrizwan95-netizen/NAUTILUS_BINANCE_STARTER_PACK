...
def run_backfill() -> None:
    logger.info("Starting Binance backfill job for symbols={} timeframe={}", settings.SYMBOLS, settings.TIMEFRAME)
    ...
    for symbol in settings.symbol_list:
        ...
        logger.info("Backfilling {} from {} (target end {})", symbol, since, target_end)
        downloaded = 0
        ...
        while since < target_end:
            try:
                ...
                logger.warning("Fetch error for {} at {}: {}", symbol, since, exc)
                _sleep()
                continue

            if not candles:
                logger.info("No more candles for {}; stopping.", symbol)
                break
            ...
            if frame.empty:
                logger.info("All fetched candles exceed target end for {}; stopping.", symbol)
                break
            ...
            if written:
                ...
                logger.info(
                    "Saved {} rows for {} [{} -> {}] (total downloaded={})",
                    len(frame),
                    symbol,
                ...
                logger.debug("Duplicate chunk ignored for {} at {}", symbol, next_since)

            _sleep()

        logger.info("Finished backfill for {} (downloaded {} rows)", symbol, downloaded)
