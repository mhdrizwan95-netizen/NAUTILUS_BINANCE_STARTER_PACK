...
def ingest_once():
    ...
    for sym in [s.strip() for s in settings.SYMBOLS.split(",") if s.strip()]:
        ...
        since = manifest.get_watermark(wm_name, default=settings.START_TS, db_path=settings.LEDGER_DB)
        if since is None:
            since = 0
        # ccxt expects ms
        ...
        try:
            ...
            file_id, inserted = manifest.register_file(
                path=path, symbol=sym, timeframe=tf, t_start=t_start, t_end=t_end, db_path=settings.LEDGER_DB
            )
            if inserted:
