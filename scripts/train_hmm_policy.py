...
import argparse, os, json, math, pickle, warnings
...
def main(cfg: TrainConfig):
    ...
    feats = _rolling_features(df["price"].values.astype(float),
    ...
    # HMM fit
    hmm = GaussianHMM(n_components=cfg.states,
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    cfg = TrainConfig(csv=args.csv, symbol=args.symbol, window=args.window, states=args.states,
                      seed=args.seed, out=args.out, min_rows=args.min_rows, cov=args.cov, stride=args.stride)
    main(cfg)
