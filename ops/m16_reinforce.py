...
def weights(df: pd.DataFrame) -> np.ndarray:
    ...
    risk_pen = 1.0 + LAMBDA*np.clip(df["drawdown_state"].values, 0, None) + MU*np.clip(df["spread_bp"].values, 0, None)
...
def kl_divergence(p_old: np.ndarray, p_new: np.ndarray) -> float:
    eps = 1e-8
    p_old = np.clip(p_old, eps, 1.0); p_new = np.clip(p_new, eps, 1.0)
...
def guarded_update(df: pd.DataFrame) -> Dict:
    ...
    policy = SGDClassifier(loss="log_loss", learning_rate="constant", eta0=LR, max_iter=1, warm_start=True)
    ...
    if kl > KL_MAX:
        ...
        policy = SGDClassifier(loss="log_loss", learning_rate="constant", eta0=LR*tau, max_iter=200, alpha=1e-3)
        policy.fit(X, Y)
    ...
    plt.plot(rw.values); plt.title("M16 Learning Curve (γ-weighted reward)"); plt.ylabel("reward"); plt.xlabel("t")
    plt.tight_layout(); plt.savefig(lc_path, dpi=160); plt.close()
    ...
    plt.xlabel("drift_score"); plt.ylabel("robust_reward")
    plt.title("Drift vs Reward")
    plt.tight_layout(); plt.savefig(dvr_path, dpi=160); plt.close()

    # Prometheus exposition to file
    reg = CollectorRegistry()
    g1 = Gauge("m16_avg_reward", "avg recency-weighted reward", registry=reg); g1.set(avg_reward)
    g2 = Gauge("m16_entropy", "policy average entropy", registry=reg); g2.set(entropy)
    g3 = Gauge("m16_winrate", "trade win rate", registry=reg); g3.set(winrate)
