...
class EMATable:
    ...
    def update(self, key, reward: float):
        a = self.alpha
        self.avg[key] = (1 - a) * self.avg[key] + a * reward
...
for _, r in df.iterrows():
    R.update((r.macro_state, r.micro_state), r.delta_pnl)
...
plt.ylabel("ΔPnL")
plt.tight_layout()
plt.savefig(CAL_DIR / "reward_heatmap.png")
plt.close()
...
def feats(row):
    ...
    m = int(row["macro_state"])
    s = int(row["micro_state"])
    mvec = np.zeros(4)
    svec = np.zeros(8)
    mvec[min(m, 3)] = 1
    svec[min(s, 7)] = 1
...
y = np.array(
    [(1 if (r.side == "BUY" and r.delta_pnl > 0) else 0) for _, r in df.iterrows()],
    dtype=float,
)
...
for i in range(0, len(X), 100):
    j = min(i + 64, len(X))
    mlp.step(X[i:j], y[i:j])
    probe = float(mlp.predict_proba(X[i:j].mean(0)[None, :])[0])
    probe_log.append(probe)

plt.figure(figsize=(8, 4))
plt.plot(probe_log)
plt.title("TinyMLP probe p(BUY) over time")
plt.xlabel("batch index")
plt.ylabel("p(BUY)")
plt.tight_layout()
plt.savefig(CAL_DIR / "policy_boundary.png")
plt.close()

# --- Rolling win-rate (on executed BUY/SELL only) ---
df_side = df[df.side.isin(["BUY", "SELL"])].copy()
df_side["win"] = df_side["delta_pnl"] > 0
win = pd.Series(df_side["win"].astype(int)).rolling(100, min_periods=25).mean()

plt.figure(figsize=(8, 3))
plt.plot(win.values)
plt.title("Rolling win-rate (100 trades)")
plt.ylim(0, 1)
plt.xlabel("trade index")
plt.ylabel("win-rate")
plt.tight_layout()
plt.savefig(CAL_DIR / "rolling_winrate.png")
plt.close()
