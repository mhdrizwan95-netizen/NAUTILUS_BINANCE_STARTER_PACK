...
def load_lineage_data() -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        df["winrate_last"] = df.apply(
            lambda row: row.get("performance_snapshot", {}).get("winrate_last", 0), axis=1
        )
...
def create_lineage_graph(df: pd.DataFrame, output_path: str) -> None:
    ...
    # Plot P&L trend line
    ax1.plot(df["archived_at"], df["pnl_last"],
             linewidth=2, alpha=0.7, color="#1f77b4",
    ...
    for gen_type, group in df.groupby("generation_type"):
        ...
        ax1.scatter(group["archived_at"], group["pnl_last"],
                   color=color, s=80, alpha=0.8, edgecolors='white',
                   linewidth=2, label=f"{gen_type} generations")

    ax1.set_ylabel("Realized P&L (USD)", fontsize=12, fontweight='bold')
    ax1.grid(True, alpha=0.3)
    ax1.set_title("Evolutionary Lineage & Performance Trajectory\nTrading Organism M15→M21",
                 fontsize=14, fontweight='bold', pad=20)

    # Secondary axis: Winrate performance
    ax2 = ax1.twinx()
    ax2.plot(df["archived_at"], df["winrate_last"],
    ...
    if len(df) >= 3:
        ...
        pnl_trend = ax1.axhline(y=df["pnl_last"].mean(), color='#1f77b4',
                               linestyle='--', alpha=0.5,
                               label=f'Avg P&L: ${df["pnl_last"].mean():.2f}')

        # Linear trend for winrate
        winrate_trend = ax2.axhline(y=df["winrate_last"].mean(), color='#ff7f0e',
                                   linestyle='--', alpha=0.5,
...
def add_summary_stats(df: pd.DataFrame, ax) -> None:
    ...
    basic_stats = {
        ...
        "Evolution Span": f"{((df['archived_at'].max() - df['archived_at'].min()).total_seconds() / 86400):.0f} days" if len(df) > 1 else "N/A",
...
def create_performance_distribution(df: pd.DataFrame, output_path: str) -> None:
    ...
    ax1.axvline(df["pnl_last"].mean(), color='red', linestyle='--', linewidth=2,
               label=f'Mean: ${df["pnl_last"].mean():.2f}')
    ax1.axvline(df["pnl_last"].median(), color='green', linestyle='--', linewidth=2,
    ...
    ax2.axvline(df["winrate_last"].mean(), color='red', linestyle='--', linewidth=2,
               label=f'Mean: {df["winrate_last"].mean():.1f}%')
    ax2.axvline(df["winrate_last"].median(), color='green', linestyle='--', linewidth=2,
...
def create_generation_type_summary(df: pd.DataFrame, output_path: str) -> None:
    ...
    gen_stats = df.groupby("generation_type").agg({
        "pnl_last": ["mean", "max", "count"],
        "winrate_last": ["mean", "max"]
    }).round(2)
    ...
    plt.suptitle("Generation Type Performance Comparison\nWhich evolutionary stages drove improvement?")
    plt.tight_layout()
    plt.savefig(output_path, bbox_inches='tight')
