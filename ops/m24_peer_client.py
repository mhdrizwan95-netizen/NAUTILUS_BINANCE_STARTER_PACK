...
def share_summary():
    ...
    try:
        ...
        open(SYNC_LOG,"a").write(json.dumps({"time":datetime.utcnow().isoformat(),"sent":payload,"status":r.status_code})+"\n")
    except Exception as e:
        open(SYNC_LOG,"a").write(json.dumps({"time":datetime.utcnow().isoformat(),"error":str(e)})+"\n")
...
def pull_consensus():
    try:
        ...
        open(SYNC_LOG,"a").write(json.dumps({"time":datetime.utcnow().isoformat(),"pull_error":str(e)})+"\n")
