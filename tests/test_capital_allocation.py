...
class TestCapitalAllocation:
    ...
    def temp_allocations_file(self, tmp_path):
        ...
        allocations = {
            "capital_quota_usd": {"hmm_v2_canary": 5000.0, "ma_crossover_v3": 3000.0},
    ...
    def test_quota_bounds_enforcement(self):
        ...
        try:
            ...
            assert (
                quota <= policy["max_quota_usd"]
            ), f"Quota {quota} exceeds max {policy['max_quota_usd']}"
            assert (
                quota >= policy["min_quota_usd"]
            ), f"Quota {quota} below min {policy['min_quota_usd']}"
        finally:
