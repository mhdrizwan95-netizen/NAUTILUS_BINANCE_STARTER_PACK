...
"""
    ...
    with respx.mock:
        respx.get("http://engine_binance:8003/metrics").mock(
            return_value=Response(200, text=metrics_response)
        )
...
async def test_push_model_update(monkeypatch):
    ...
    with respx.mock:
        ...
        respx.post("http://engine_binance:8003/strategy/promote").mock(
            return_value=Response(200, json={"status": "ok"})
        )
        respx.post("http://engine_ibkr:8005/strategy/promote").mock(
            return_value=Response(200, json={"status": "ok"})
        )
