...
def test_strategy_signal_dry_run():
    ...
    c = mk_client()
    r = c.post("/strategy/signal", json={
        "symbol": "BTCUSDT.BINANCE", "side": "BUY", "quote": 10, "tag": "test"
    })
...
def test_strategy_signal_respects_rails():
    ...
    c = mk_client()
    r = c.post("/strategy/signal", json={
        "symbol": "DOGEUSDT.BINANCE", "side": "BUY", "quote": 10
    })
    assert r.status_code == 200
...
def test_strategy_signal_override_dry_run():
    ...
    c = mk_client()
    r = c.post("/strategy/signal", json={
        "symbol": "BTCUSDT.BINANCE", "side": "BUY", "quote": 10, "dry_run": True
    })
    assert r.status_code == 200
