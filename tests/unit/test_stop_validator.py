...
class _MD:
    def atr(self, s, tf="1m", n=14): return 0.2
    def last(self, s): return 10.0
    ...
    def __init__(self):
        class _Pos:
            def __init__(self):
                ...
                self.positions = {"ABCUSDT": Position(symbol="ABCUSDT", quantity=100.0, avg_price=10.0)}
        self._state = _Pos()
...
async def test_stop_validator_repairs_missing_sl():
    sv = StopValidator({"STOP_VALIDATOR_ENABLED": True, "STOP_VALIDATOR_REPAIR": True}, _Router(), _MD(), log=types.SimpleNamespace(warning=lambda *a, **k: None), metrics=_Metrics())
