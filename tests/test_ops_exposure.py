...
from ops.aggregate_exposure import aggregate_exposure, Position, AggregateExposure
...
async def test_exposure_aggregates_binance_and_ibkr(monkeypatch):
    ...
    bin_port = {"positions":[
        {"symbol":"BTCUSDT.BINANCE","qty_base":0.002,"last_price_quote":50000.0}
    ]}
    # Mock IBKR portfolio (AAPL shares, no price in portfolio)
    ibkr_port = {"positions":[
        {"symbol":"AAPL.IBKR","qty_base":2.0}
    ]}
    ...
    with respx.mock:
        ...
        class FakeGauge:
            ...
            @property
            def _value(self): return FakeGauge._Val(self._v)
        ...
        agg = await aggregate_exposure(engine_endpoints_env="http://e_bin:8003,http://e_ibkr:8005",
...
async def test_exposure_handles_unavailable_engines():
    ...
    missing_portfolio = {"positions": [{"symbol": "ETHUSDT.BINANCE", "qty_base": 1.0, "last_price_quote": 2500.0}]}

    with respx.mock:
        # One engine available, one unavailable
        respx.get("http://good:8003/portfolio").mock(return_value=Response(200, json=missing_portfolio))
...
def test_exposure_api_endpoint_integration(monkeypatch):
    ...
    respx.get("http://e_bin:8003/portfolio").mock(
        return_value=Response(200, json={
            "positions":[{"symbol":"BTCUSDT.BINANCE","qty_base":0.001,"last_price_quote":60000.0}]
        })
    )
