...
def test_canary_max_weight_enforcement(sample_weights):
    ...
    original_canary_weight = config['weights']['canary_model']
    ...
    assert canary_pct <= config['max_canary_weight'] + 0.02, \
...
def test_weights_file_parsing():
    ...
    required_fields = ['current', 'weights', 'min_trades_for_eval',
                      'min_live_hours', 'max_canary_weight']
