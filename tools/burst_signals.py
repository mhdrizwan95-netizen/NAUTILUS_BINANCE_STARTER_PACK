...
def main(pipeline=None) -> None:
    if pipeline is None:
        raise RuntimeError("burst_signals CLI expects to be invoked from synthetic harness with a pipeline")
    args = parse_args()
    asyncio.run(burst(pipeline, args.strategy, args.symbol, args.count, args.delay, args.side, args.confidence))
