...
class DummyClient:
    ...
    async def submit_limit_order(
        self, symbol: str, side: str, quantity: float, price: float, **_kwargs
    ):
...
class SyntheticExchange:
    ...
    def _advance_symbol(self, sym: str) -> SymbolMetrics:
        ...
        volume = abs(drift) * 5_000_000 + random.uniform(
...
async def run_synthetic_screener(
    ...
    while True:
        ...
        for strategy, filter_cfg in universes.items():
            symbols = UniverseScreener._apply_filter(
...
async def run_synthetic_runtime(args: argparse.Namespace) -> None:
    ...
    exchange = SyntheticExchange(
        symbols, base_price=args.base_price, spike_chance=args.spike_chance
    )
    ...
    consumer_task = asyncio.create_task(
        pipeline._consumer(), name="synthetic-consumer"
    )  # noqa: SLF001
    ...
    signal_task = asyncio.create_task(
        synthetic_signal_stream(
            manager,
            pipeline,
            (config.universes or {}).keys(),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config", default="config/runtime.yaml", help="Path to runtime YAML config"
    )
